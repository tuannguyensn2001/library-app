...
@section('module')
    ...
    <li class="breadcrumb-item active"><p>{{trans('books.edit')}}</p></li>
@endsection
    ...
    <div class="container">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">{{trans('books.edit')}}</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form id="form_create_book" method="post" action="{{route('books.update',['book' => $book->id])}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <div class="card-body">
                    <div class="form-group">
                        <label for="name">{{trans('books.name')}}</label>
                        <input type="text" name="name" id="name" class="form-control" value="{{old('name',$book->name)}}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select value="{{$book->language}}" class="custom-select form-control" id="language" name="language"
                                value="{{old('language')}}">
                    ...
                    <div class="form-group">
                        ...
                        <select  value="{{$book->category_id}}" class="custom-select form-control" id="category_id" name="category_id"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="quantity" id="quantity" class="form-control" value="{{old('quantity',$book->quantity)}}">
                    </div>
                </div>
                <!-- /.card-body -->

                <div class="card-footer d-flex justify-content-end">
                    <button type="submit" class="btn btn-flat btn-primary">{{trans('action.submit_edit')}}</button>
                </div>
            </form>
        </div>
